...
def plot_opi_distr(G):
    ...
    plt.title("Maximum frequency= " + str(y.max()))
    plt.show()
...
def run3(graph1, d1, graph2, d2, graph3, d3, e):  # runs deffaunt model for 3 layers, e is break parameter
    ...
    while True:
        ...
        if itera % (20 * nx.number_of_nodes(graph1)) == 0:  # check if stable at every 10*number of nodes iterations
...
if __name__ == "__main__":
    ...
    for i in [0.5]:#[0.5,0.6,0.7,0.8]:
        g1 = copy.deepcopy(graphs_dict[i][1])
        g2 = copy.deepcopy(graphs_dict[i][2])
        itera = -1
        
        for j in [0.0]:#, 0.1]: #(2*i-1, i+0.1, 0.1):
            itera += 1
            g3 = copy.deepcopy(graphs_dict[i][3][round(j,1)])
            
            for d1 in range(0,11,1): #builds yaxis
                d1/=100
                d2 = (0.1 - d1)
                print("Running for alpha = beta = " + str(i) + " , gamma = " + str(round(j,1)) + " , d1 = " + str(round(d1,2)) + " , d2 = " + str(round(d2,2)) + " , d3 = " + str(round(d3,2)))
